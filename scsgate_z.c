...
// {"coverpct":"false","devclear":"true","extended":"true","busid":"00"}
	  ...
	  if ((strcmp(extend,"true") == 0) && (uartgate)) 
	  {
		...
		write(fduart,PIC_START,1);		// scrittura su scsgate
		write(fduart,"O1",2);			// scrittura su scsgate
		char c = my_busid+'0';		// 
		...
		// bit per bit analizza i cambiati
				...
				while (scan)
				{
					if (scan & chgd)		// scan (00000001-10000000)/ ixd (0-7)     indicano un bit cambiato 
					{
						...
						// se il tipo di indirizzo comandato § SCS (1 o 3) schedula il comando
...
//  scs:  0-1-2=pulsante      3-4-5=deviatore
						...
						// se il tipo di indirizzo comandato § i2c (da 30 a 3F) genera il comando
						else
