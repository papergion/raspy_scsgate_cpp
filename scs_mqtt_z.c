...
//    dev[3] = *(topicName + sizeof(SWITCH_SET)+2);
	memcpy(dev, (topicName + sizeof(SWITCH_SET) - 1), 4);
	dev[4] = 0;
    device = (uint16_t)strtoul(dev, &ch, 16);
...
//    device = (char)strtoul(dev, &ch, 16);
	memcpy(dev, (topicName + sizeof(BRIGHT_SET) - 1), 4);
	dev[4] = 0;
    device = (uint16_t)strtoul(dev, &ch, 16);
    if (memcmp(payLoad, "ON", 2) == 0)
      command = 0x00;
...
//    dev[2] = 0;
//    device = (char)strtoul(dev, &ch, 16);
	memcpy(dev, (topicName + sizeof(COVER_SET) - 1), 4);
	dev[4] = 0;
    device = (uint16_t)strtoul(dev, &ch, 16);
...
//    dev[2] = 0;
//    device = (char)strtoul(dev, &ch, 16);
	memcpy(dev, (topicName + sizeof(COVERPCT_SET) - 1), 4);
	dev[4] = 0;
    device = (uint16_t)strtoul(dev, &ch, 16);
...
//    dev[2] = 0;
//    device = (char)strtoul(dev, &ch, 16);
	memcpy(dev, (topicName + sizeof(GENERIC_SET) - 1), 4);
	dev[4] = 0;
    device = (uint16_t)strtoul(dev, &ch, 16);
...
//    dev[2] = 0;
//    device = (char)strtoul(dev, &ch, 16);
	memcpy(dev, (topicName + sizeof(SWITCH_STATE) - 1), 4);
	dev[4] = 0;
    device = (uint16_t)strtoul(dev, &ch, 16);
...
// ---------------------------------------------------------------------------------------------------------------------------------
 ...
 // START pubblicazione stato device        [0xF5] [y] 32 00 12 01
	...
	printf("MQTTR %s t:%02x c:%02x v:%d\n",nomeDevice,busdata->bustype,busdata->buscommand,busdata->busvalue);
...
// ================================ INTERPRETAZIONE STATO ===========================================
  ...
  // ----------------------------------pubblicazione stati GENERIC device SCS (to & from)------------------------
	...
	if (busdata->bustype == 12)  
	{
		...
		strcat(topic, nomeDevice);
		sprintf(payload, "%s%02X%02X", nomeDevice, busdata->busrequest, busdata->buscommand);
		publish(topic, payload, 1);
		printf("%s -> %s\n",topic,payload);
    }
	else
...
//    [F3] u <address> <position%>
	...
	if (devtype == 0)
	{
		...
		sprintf(payload, "to %s, from %02X, req %02X, cmd %02X, val %d", nomeDevice, busdata->busfrom, busdata->busrequest, busdata->buscommand, busdata->busvalue);
...
// ===================================================================================
char MQTTrequestExt(bus_scs_queue * busdata)
// ===================================================================================
// è arrivato un messaggio SCS
// input: dati del messaggio e tipo dispositivo censito
// output: se necessario topic di stato
...
// ---------------------------------------------------------------------------------------------------------------------------------
 ...
 // START pubblicazione stato device        [0xF5] [y] 32 00 12 01
	...
	printf("MQTTR %04x t:%02x c:%02x v:%d\n",busdata->busaddress,busdata->bustype,busdata->buscommand,busdata->busvalue);
...
// ================================ INTERPRETAZIONE STATO ===========================================
  ...
  // ----------------------------------pubblicazione stati GENERIC device SCS (to & from)------------------------
	...
	if (busdata->bustype == 12)  
	{
		...
		strcpy(topic, GENERIC_FROM);
		strcat(topic, nomeDevice);
		sprintf(payload, "%04X%02X%02X", busdata->busaddress, busdata->busrequest, busdata->buscommand);
		publish(topic, payload, 1);
		printf("%s -> %s\n",topic,payload);
    }
	else
...
//    [F3] u <address> <position%>
	...
	{
		strcpy(topic, "NO_TOPIC");
		sprintf(payload, "to %04X, from %02X, req %02X, cmd %02X, val %d", busdata->busaddress, busdata->busfrom, busdata->busrequest, busdata->buscommand, busdata->busvalue);
		publish(topic, payload, 0); 
		printf("%s -> %s\n",topic,payload);
	} 
//	printf("end mqttrequest\n");
...
// ================================ INTERPRETAZIONE COMANDO/STATO ===========================================
  ...
  // ----------------------------------pubblicazione stati GENERIC device SCS (to & from)------------------------
	...
	if (busdata->bustype == 12)  
	{
		...
		strcpy(topic, GENERIC_SET);
		strcat(topic, nomeDevice);
		sprintf(payload, "%02X%02X%02X", busdata->busaddress, busdata->busrequest, busdata->buscommand);
		publish(topic, payload, 0);
		printf("%s -> %s\n",topic,payload);
    }
	else
...
//    [F3] u <address> <position%>
	...
	{
		strcpy(topic, "NO_TOPIC");
		sprintf(payload, "to %02X, from %02X, req %02X, cmd %02X, val %d", busdata->busaddress, busdata->busfrom, busdata->busrequest, busdata->buscommand, busdata->busvalue);
		publish(topic, payload, 0); 
		printf("%s -> %s\n",topic,payload);
	} 
//	printf("end mqttrequest\n");
...
// ===================================================================================
char MQTTcommandExt(bus_scs_queue * busdata)
// ===================================================================================
// richiesta di pubblicazione comando in mqtt
// input: dati del messaggio e tipo dispositivo censito
// output: topic di comando
